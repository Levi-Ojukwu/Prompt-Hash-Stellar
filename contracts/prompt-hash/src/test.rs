...
fn test_create_prompt_stores_encrypted_fields() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Secure Prompt", 10_000_000, &context.xlm);
...
fn test_creator_can_pause_reactivate_and_update_price() {
    ...
    let creator = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Pricing Prompt", 5_000, &context.xlm);
...
fn test_buy_prompt_grants_access_to_multiple_buyers_and_tracks_exact_fees() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Reusable Prompt", 12_345, &context.xlm);
...
fn test_has_access_is_true_for_creator_and_buyer_but_not_stranger() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Access Prompt", 8_000, &context.xlm);
...
fn test_creator_cannot_buy_own_prompt() {
    ...
    let creator = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Creator Lockout", 4_000, &context.xlm);
...
fn test_inactive_prompt_cannot_be_bought() {
    ...
    let creator = Address::generate(&env);
    let buyer = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Paused Prompt", 4_000, &context.xlm);
...
fn test_buy_prompt_with_zero_fee() {
    ...
    let buyer = Address::generate(&env);
    let price = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Zero Fee Prompt", price, &context.xlm);
...
fn test_buy_prompt_with_max_fee() {
    ...
    let buyer = Address::generate(&env);
    let price = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Max Fee Prompt", price, &context.xlm);
...
fn test_unauthorized_seller_actions_fail() {
    ...
    let creator = Address::generate(&env);
    let stranger = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Protected Prompt", 5_000, &context.xlm);
...
fn test_global_pause_blocks_mutations_but_not_reads() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Readable Prompt", 10_000, &context.xlm);
    client.set_pause_status(&true);
...
fn test_buy_prompt_with_referrer_splits_payment_correctly() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Referral Prompt", price, &context.xlm);
...
fn test_referrer_cannot_be_buyer() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Self Referral Prompt", price, &context.xlm);
...
fn test_referrer_cannot_be_creator() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Creator Referral Prompt", price, &context.xlm);
...
fn test_buy_without_referrer_no_referral_amount_paid() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "No Referral Prompt", price, &context.xlm);
...
fn test_buy_prompt_blocked_when_paused() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Pausable Prompt", price, &context.xlm);
...
fn test_update_prompt_price_blocked_when_paused() {
    ...
    let creator = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Price Update Prompt", 5_000, &context.xlm);
...
fn test_read_only_methods_work_when_paused() {
    ...
    let creator = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Read Only Prompt", 5_000, &context.xlm);
...
fn test_unpause_restores_operations() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 5_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Unpause Prompt", price, &context.xlm);
...
fn test_tip_above_price_succeeds_and_creator_receives_full_tip() {
    ...
    let prompt_id = create_prompt(&env, &client, &creator, "Tippable Prompt", price, &context.xlm);
...
fn test_payment_below_price_fails() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Underpay Prompt", price, &context.xlm);
...
fn test_exact_price_payment_succeeds() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Exact Pay Prompt", price, &context.xlm);
...
fn test_voucher_applies_discount_on_purchase() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Voucher Prompt", price, &context.xlm);
...
fn test_voucher_is_single_use_second_use_fails() {
    ...
    let buyer_two = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Single Use Voucher", price, &context.xlm);
...
fn test_invalid_voucher_code_fails() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Invalid Voucher Prompt", price, &context.xlm);
...
fn test_only_creator_can_add_voucher() {
    ...
    let creator = Address::generate(&env);
    let stranger = Address::generate(&env);
    let prompt_id = create_prompt(&env, &client, &creator, "Voucher Auth Prompt", 5_000, &context.xlm);
...
fn test_creator_can_remove_voucher() {
    ...
    let buyer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Remove Voucher Prompt", price, &context.xlm);
...
fn test_voucher_with_referrer_combined() {
    ...
    let referrer = Address::generate(&env);
    let price: i128 = 10_000;
    let prompt_id = create_prompt(&env, &client, &creator, "Voucher+Referral Prompt", price, &context.xlm);
